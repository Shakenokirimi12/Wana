...
function runPackageDeploy(absDir) {
 ...
 */
function patchConfigValue(file, key, value) {
    if (!existsSync(file))
        return false;
...
cli
    .command("provision", "Create Cloudflare resources (D1/R2/Queues/KV) and write their ids into wrangler.jsonc")
    ...
    for (const { label, name, args } of simpleResources) {
        ...
        console.log(`${r.ok || exists ? "✓" : "✗"} ${label}: ${name}${exists ? " (already exists)" : ""}`);
        if (!r.ok && !exists) {
            console.error(r.out);
            process.exit(1);
        }
    ...
    let kvId = "";
    {
        ...
        if (!kvId) {
            ...
            const entry = list.out.match(new RegExp(`"id"\\s*:\\s*"([0-9a-f]{32})"[^}]*"title"\\s*:\\s*"[^"]*${RESOURCES.kvBinding}"`, "i"));
            kvId = entry?.[1] ?? "";
        }
        if (!kvId) {
    ...
    const mig = run("pnpm", ["exec", "wrangler", "d1", "migrations", "apply", RESOURCES.d1Name, "--remote"], INGEST_DIR);
    console.log(mig.out);
    if (!mig.ok) {
 ...
 */
function deriveDashboardOrigin(ingestHost) {
...
function readGitContext(cwd) {
    ...
    if (remoteRes.ok) {
        ...
        const m = remoteRes.out.match(/github\.com[:/]+([A-Za-z0-9._-]+)\/([A-Za-z0-9._-]+?)(?:\.git)?$/);
        if (m)
            repo = `${m[1]}/${m[2]}`;
    }
    return { sha, repo };
...
function machoUuid(path) {
    ...
    if (!r.ok)
        return null;
    const m = r.out.match(/UUID:\s*([0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12})\s*\(([^)]+)\)/i);
    if (!m)
        return null;
    return { uuid: m[1].replace(/-/g, "").toLowerCase(), arch: m[2] || null };
}
async function uploadOne(apiUrl, projectId, publicKey, path, git) {
    const bytes = readFileSync(path);
    const form = new FormData();
    form.set("file", new Blob([new Uint8Array(bytes)], { type: "application/octet-stream" }), basename(path));
    if (git.sha)
        form.set("git_sha", git.sha);
    if (git.repo)
        form.set("git_repo", git.repo);
...
cli
    .command("upload-dif [path]", "Upload dSYM / .debug.dylib symbol files to Wana. Defaults to scanning the cwd.")
    .option("--dsn <url>", "Project DSN. Falls back to $WANA_DSN or $SENTRY_DSN.")
    .option("--api-url <url>", "Override dashboard origin. Defaults to DSN host with `ingest.` stripped.")
    .option("--git-sha <sha>", "Git SHA. Defaults to `git rev-parse HEAD`.")
    .option("--git-repo <owner/repo>", "GitHub `owner/repo`. Defaults to parsing `git remote get-url origin`.")
    .action(async (path, options) => {
    const dsnRaw = options.dsn ?? process.env.WANA_DSN ?? process.env.SENTRY_DSN;
    const dsn = parseDsn(dsnRaw);
    if (!dsn) {
        console.error("✗ Missing/invalid DSN. Pass --dsn <url>, or set $WANA_DSN.\n" +
    ...
    for (const f of files) {
        ...
        if (!info)
            continue;
        if (!byUuid.has(info.uuid))
            byUuid.set(info.uuid, { path: f, arch: info.arch });
    }
    console.log(`Found ${byUuid.size} unique image(s). git=${git.sha?.slice(0, 7) ?? "(none)"} repo=${git.repo ?? "(none)"}`);
    let okN = 0;
    let failN = 0;
